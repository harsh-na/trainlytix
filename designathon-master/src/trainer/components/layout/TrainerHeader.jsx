...
import { TRAINER, SIDEBAR_ITEMS } from "../../lib/mockData";
import { useAuth } from "../../../context/AuthContext";

const TrainerHeader = ({ onMenuClick }) => {
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="relative hidden md:block">
            ...
            <input
              ...
              placeholder="Search sessions, batches, trainees..."
              className="w-72 h-9 rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            />
          </div>
          ...
          >
            ...
            <span className="absolute -top-1 -right-1 h-4 min-w-[16px] px-1 rounded-full bg-purple-500 text-[10px] font-bold text-white flex items-center justify-center">
              3
            </span>
          </button>
          ...
          <div className="relative">
            ...
            >
              <div className="h-8 w-8 bg-purple-600 rounded-full flex items-center justify-center text-white text-xs font-semibold">
                {initials}
              </div>
              <div className="hidden sm:block text-left">
                <p className="text-sm font-semibold text-slate-900 leading-none">{TRAINER.name.split(" ")[0]}</p>
                <p className="text-xs text-slate-500 leading-none mt-1">Trainer</p>
              </div>
            </button>
            ...
            {showMenu && (
              <div className="absolute top-full right-0 mt-1 w-56 bg-white border border-slate-200 rounded-lg shadow-lg z-50">
                ...
                <button 
                  ...
                  onClick={() => { navigate("/trainer/profile"); setShowMenu(false); }}
