...
const TrainerSidebar = ({ collapsed, setCollapsed, mobileOpen, setMobileOpen }) => {
  return (
    <>
      {mobileOpen && (
        <div
          data-testid="trainer-sidebar-backdrop"
          className="fixed inset-0 z-30 bg-slate-900/50 lg:hidden"
          onClick={() => setMobileOpen(false)}
        />
      )}
      <aside
        data-testid="trainer-sidebar"
      ...
      >
        <div className="flex items-center justify-between px-5 py-5 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            <div className="h-9 w-9 rounded-lg bg-purple-600 flex items-center justify-center shadow-lg shadow-purple-600/30">
            ...
            {!collapsed && (
              <div className="flex flex-col">
                ...
                <span className="text-[10px] uppercase tracking-widest text-slate-500 mt-1">Trainer Portal</span>
              </div>
            )}
          </div>
          <button
            data-testid="trainer-sidebar-collapse-btn"
          ...
          {SIDEBAR_ITEMS.map((item) => {
            ...
            return (
              <NavLink
                key={item.key}
                to={item.path}
                data-testid={`nav-trainer-${item.key}`}
                onClick={() => setMobileOpen(false)}
                className={({ isActive }) =>
                  `group relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all
                  ${isActive
                    ? "bg-slate-800 text-white border-l-[3px] border-purple-500 pl-[9px]"
        ...
        <div className={`p-4 border-t border-slate-800 ${collapsed ? "hidden lg:block" : ""}`}>
          {!collapsed ? (
            <div className="rounded-xl bg-gradient-to-br from-slate-800 to-slate-800/40 p-4 border border-slate-700/50">
              ...
              <p className="text-sm text-white font-medium leading-snug mb-3">Contact support team</p>
              <button className="text-xs font-semibold text-purple-400 hover:text-purple-300">support@trainlytix.io →</button>
            </div>
          ) : (
