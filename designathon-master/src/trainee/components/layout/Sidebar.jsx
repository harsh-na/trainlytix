          ...
          <div className="flex items-center gap-2.5">
            ...
            {!collapsed && (
              <div className="flex flex-col">
                ...
                <span className="text-[10px] uppercase tracking-widest text-slate-500 mt-1">Trainee Portal</span>
              </div>
            )}
          </div>
          <button
        ...
        <div className={`p-4 border-t border-slate-800 ${collapsed ? "hidden lg:block" : ""}`}>
          {!collapsed ? (
            <div className="rounded-xl bg-gradient-to-br from-slate-800 to-slate-800/40 p-4 border border-slate-700/50">
              ...
              <p className="text-sm text-white font-medium leading-snug mb-3">Contact your training coordinator</p>
              <button className="text-xs font-semibold text-blue-400 hover:text-blue-300">support@trainlytix.io →</button>
            </div>
          ) : (
            <div className="flex justify-center">
              <Icons.LifeBuoy className="h-5 w-5 text-slate-500" />
            </div>
          )}
        </div>
      </aside>
